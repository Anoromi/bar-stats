...
// import init from "~/rstar/pkg/bar_stats_wasm_bg.wasm?init";
...
<template>
  <article class="flex flex-1 flex-col items-center p-4">
    <div class="flex w-full flex-1 flex-col xl:flex-row xl:justify-center">
      <GeneralForm :initial-values="initialData" :on-submit="onSubmit"
        :center="pendingParams === null && results === undefined">
      </GeneralForm>
      <LazyGeneralDataDisplay v-if="results !== undefined && results.response.data.battles.length > 0"
        :results="results.response" :search-params="results.params">
      </LazyGeneralDataDisplay>
      <div v-else-if="results !== undefined" class="ml-[32rem] mt-20 flex w-full text-2xl font-bold">
