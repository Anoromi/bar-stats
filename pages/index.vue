...
import { CheckIcon, ChevronDownIcon, MagnifyingGlassIcon } from '@radix-icons/vue'
import type { CachedUsersResponse } from '~/server/api/cached-users';
import { Combobox } from 'radix-vue/namespaced';
import { ComboboxAnchor, ComboboxContent, ComboboxEmpty, ComboboxGroup, ComboboxInput, ComboboxItem, ComboboxItemIndicator, ComboboxLabel, ComboboxRoot, ComboboxSeparator, ComboboxViewport } from 'radix-vue';
...
<template>
  <article class=" flex flex-col items-center">
    <div class="sm:max-w-[70rem] pt-10 sm:pt-[30%] flex flex-col">
      <ComboboxRoot v-model:search-term="searchedUser"
        v-model:open="open">
        <ComboboxAnchor :class="cn('sm:w-[300px] flex gap-x-2 items-center bg-primary text-primary-foreground rounded-full px-6 py-3 hover:bg-primary/80 border border-primary transition-colors duration-300',
          {
          ...
          <ComboboxInput
            ...
            :class="cn('placeholder-primary-foreground', {
              'placeholder-foreground': open
            })" placeholder="Search user..." @click="open = true" />
        ...
        <ComboboxContent
          ...
          <ComboboxViewport class="p-[5px]">
            <ComboboxEmpty class="font-medium text-center py-2">
              {{
                searchedUser.length < 3 ? 'Type at least 2 characters'
                  : 'No users found' }} </ComboboxEmpty>
                <ComboboxGroup class="py-2 px-1">
                  <ComboboxItem v-for="(option, index) in users"
                    :key="option.username" :value="option"
