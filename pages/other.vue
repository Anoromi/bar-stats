    ...
    <div class="sm:max-w-[70rem] pt-10 sm:pt-[30%] flex flex-col">
      <ComboboxRoot v-model:search-term="searchedUser" v-model:open="open">
        <ComboboxAnchor :class="cn(
          ...
          ">
          <ComboboxInput class="bg-transparent outline-none text-xl min-w-0" :class="cn('placeholder-primary-foreground', {
        ...
        <ComboboxContent
          ...
          <ComboboxViewport class="p-[5px]">
            <ComboboxEmpty class="font-medium text-center py-2">
              {{
                searchedUser.length < 3 ? "Type at least 2 characters" : "No users found" }} </ComboboxEmpty>
                <ComboboxGroup class="py-2 px-1">
                  <ComboboxItem v-for="option in users" :key="option.username" :value="option"
