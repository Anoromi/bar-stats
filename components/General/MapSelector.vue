    ...
    <DevClientOnly>
      <Popover v-model:open="mapModelOpened">
        <PopoverTrigger as-child>
          <FormControl>
            <Button :ref="buttonId" variant="elevated" role="combobox" :class="cn(
              'justify-between',
        ...
        <PopoverContent class="popover-width w-[--radix-popover-trigger-width] bg-background p-0">
          <Command v-model:search-term="mapSearchValue" :filter-function="(v) => v">
            ...
            <CommandList>
              <CommandGroup>
                <CommandItem :key="'undefined'" :value="'undefined'" @select="() => {
                    select(undefined);
                  }
                  ">
                  <CheckIcon :class="cn(
                ...
                <CommandItem v-for="map in mapSuggestions" :key="map.id" :value="map.name" @select="() => {
                  ...
                  <CheckIcon :class="cn(
                    ...
                    " />
                  {{ map.name }}
                  <StarFilledIcon v-if="map.subclassOfId === null" :class="cn('ml-2 h-4 w-4 opacity-70')" />
                </CommandItem>
              </CommandGroup>
            </CommandList>
          </Command>
