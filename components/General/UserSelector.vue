  ...
  <FormItem class="flex flex-col">
    ...
    <DevClientOnly>
      <Popover v-model:open="opened">
        <PopoverTrigger as-child>
          <FormControl>
            <Button variant="elevated" role="combobox" :class="cn(
              'justify-between',
        ...
        <PopoverContent class="w-[--radix-popover-trigger-width] bg-background p-0">
          <Command v-model:search-term="searchValue" :filter-function="(v) => v">
            ...
            <CommandList>
              <CommandGroup>
                <CommandItem v-for="user in userSuggestions" :key="user.username" :value="user" @select="() => {
                    selectUser(user);
                  }
                  ">
                  <CheckIcon :class="cn(
                    ...
                    " />
                  <NuxtImg class="mx-2" :src="`https://flagcdn.com/16x12/${user.countryCode!.toLowerCase()}.png`"
                    :srcset="`
                        https://flagcdn.com/32x24/${user.countryCode!.toLowerCase()}.png 2x,
                        https://flagcdn.com/48x36/${user.countryCode!.toLowerCase()}.png 3x
    `" width="16" height="12" :alt="user.countryCode?.slice(undefined, 2)" />
                  {{ user.username }}
                </CommandItem>
              </CommandGroup>
            </CommandList>
    ...
    <template v-if="arrayData.fields.value.length !== 0">
      <div class="flex flex-wrap gap-x-2 gap-y-2">
        <FormField v-for="(user, index) in arrayData.fields.value" :key="user.value.username"
          :name="`${name}[${index}]`">
          <div class="flex items-center rounded-lg bg-surface-high py-1 pl-6 pr-2">
            <FormLabel class="inline">
              {{ user.value.username }}
            </FormLabel>
            <Button variant="ghost" size="icon" class="rounded-full" @click.stop="() => {
